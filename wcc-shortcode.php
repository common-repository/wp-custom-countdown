...
function wcc_main_shortcode( $atts ) {
	$a = shortcode_atts( array(
		...
		$html .= $count.'"><p style="display:none;">If custom countdown is giving an error you may need to enable javascript - please visit <a href="' . plugins_url( 'support.html', __FILE__ ) . '" target="_blank">Support Center</a> for support.</p></div></div><script>(function($){ var d = new Date(); var n = d.getTime(); if( n > '.$a['now'].'){ var end = '.$h2s.' - ( n - '.$a['now'].' ) } else { var end = '.$a['now'].';}  $("#countdown'.$count.'").ClassyCountdown({theme: "';
